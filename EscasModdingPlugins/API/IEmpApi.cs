...
namespace EscasModdingPlugins
{
    ...
    public interface IEmpApi
    {
        ...
        /// <param name="useZone">The fishing zone to use. Null if the original zone will be used. "Zone" refers to the result of <see cref="GameLocation.getFishingLocation(Vector2)"/>.</param>
        ...
        /// <param name="tile">The tile to check, e.g. the position of a player's fishing bobber.</param>
        /// <param name="useLocationName">The name of the location to use instead of the provided location (e.g. the key to use in the Data/Locations asset). Null if the original location will be used.</param>
        /// <param name="useTile">The tile to use instead of the provided tile. Null if the original tile will be used.</param>
        /// <param name="useCrabPotTypes">A list of fish types that can be caught by crab pots on this tile, e.g. ["Ocean", "Freshwater"]. Null if the original types will be used.</param>
        void GetFishLocationsData(GameLocation location, Vector2 tile, out string useLocationName, out Vector2? useTile, List<string> useCrabPotTypes);
    }
}
